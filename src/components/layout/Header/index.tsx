  ...
  return (
    <div className="absolute top-0 left-0 w-full h-[5.25rem] z-[10000] pointer-events-none">
      <div className="flex justify-start items-center w-full h-full relative">
        ...
        <span className="absolute top-[2.1875rem] mt-[-.625rem] left-5 ssm:left-16 z-100 pointer-events-auto">
          <Link href="/">
        ...
        <motion.ul className="absolute pointer-events-auto top-[2.1875rem] left-[7.125rem] lg:left-[9.625rem] hidden md:flex justify-start items-center gap-x-5 lg:gap-x-8 z-110">
          {headerLinks.map((headLink) => {
            return <EachLink key={headLink.ref} headLink={headLink} />;
          })}
        </motion.ul>
        <span className="absolute pointer-events-auto top-[2rem] mt-[-.625rem] right-[4.5rem] w-9 h-9 rounded-full bg-neutral-700 text-white text-xl flex justify-center items-center">
